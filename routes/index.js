...
router.post('/reset-password', isLoggedIn, async (req, res) => {
  ...
  if (newPassword !== confirmNewPassword) {
    return res.render('reset', { error: 'New passwords do not match', footer: false });
  }
...
router.get('/message', isLoggedIn, async function (req, res) {
  const user = await userModel.findOne({ username: req.session.passport.user }).populate('followings')
...
router.get('/saved-posts', isLoggedIn, async (req, res) => {
  try {
    ...
    const savedPosts = await postModel.find({ _id: user.saved }).populate('user');
...
router.get('/feed', isLoggedIn, async (req, res) => {
  ...
  try {
    ...
    res.render('feed', { footer: true, user, posts, stories: filtered, dater: utils.formatRelativeTime, pid: posts._id });
  } catch (error) {
    console.error(error);
    res.status(500).json({ error: 'Internal Server Error' });
  }
...
router.post('/feed/:postId/comments', isLoggedIn, async function (req, res) {
  try {
    ...
    const user = await userModel.findOne({ username: req.session.passport.user }); // Adjust this based on your application's structure
...
router.post('/editcaption/:postId', isLoggedIn, async function (req, res, next) {
...
router.get('/profile', isLoggedIn, async function (req, res) {
  const user = await userModel.findOne({ username: req.session.passport.user })
    .populate("posts").populate("saved")
  res.render('profile', { footer: true, user });
});

router.get('/profile/:user', isLoggedIn, async function (req, res) {
  const userprofile = await userModel.findOne({ username: req.params.user })
    .populate("posts")
  const user = await userModel.findOne({ username: req.session.passport.user })
...
router.get('/follow/:userid', isLoggedIn, async function (req, res) {
  const followhonewala = await userModel.findOne({ _id: req.params.userid })
    .populate("posts")
...
router.post('/post', upload.single('image'), isLoggedIn, async function (req, res) {
  const user = await userModel.findOne({ username: req.session.passport.user })
